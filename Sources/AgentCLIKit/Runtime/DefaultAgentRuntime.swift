...
public actor DefaultAgentRuntime: AgentRuntime {
        ...
        try await stdinWriter.enqueue {
            ...
            do {
                ...
                try await self.writeInputDataAndAppendAcceptedSteeringEventIfNeeded(
                    ...
                    adapter: adapter,
                    context: context,
                    // The turn was already marked active above, so the write itself must not mark it again.
    ...
    private func writeInputDataAndAppendAcceptedSteeringEventIfNeeded(
        ...
        try writeInputData(
            ...
            processToken: target.processToken,
            marksTurnActive: target.marksTurnActive
        )
        if let event = acceptedSteeringInputEvent(for: input, adapter: adapter, context: context) {
            append(event, source: .runtime, conversationId: target.conversationId)
        }
    }
...
/// Routing for one stdin write: which conversation and process token it belongs to, and whether the
/// write itself should mark the turn active.
struct InputWriteTarget {
    let conversationId: AgentConversationID
    let processToken: UUID
    let marksTurnActive: Bool
}
