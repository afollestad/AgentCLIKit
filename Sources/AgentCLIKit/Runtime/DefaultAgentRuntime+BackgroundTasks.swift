...
/// Harness background tasks outlive the turn that started them, so the runtime tracks them separately from
/// `isTurnActive`: the live set feeds `AgentRuntimeStatus.liveBackgroundTaskCount`, and a dequeued notification for a
/// task this process announced becomes a harness-initiated turn so hosts see the follow-up response as a real turn.
struct BackgroundTaskTracking {
    /// Tasks from the harness's latest live-set announcement; replaced wholesale on every announcement.
    private(set) var live: [AgentBackgroundTask] = []
    /// Tasks the harness dropped from the live set before their terminal notification arrived. Claude announces the
    ...
    /// earlier process's orphans) do not start a harness-initiated turn.
...
extension DefaultAgentRuntime {
    /// Retains the activity metadata spelling used by persisted transcript events.
    static let harnessInitiatedTurnMetadataKey = "provider_initiated"
    ...
    func harnessInitiatedTurnStart(for event: AgentEvent, conversationId: AgentConversationID) -> AgentEvent? {
        guard case let .subAgent(subAgent) = event,
              ...
              !state.isTurnActive,
              state.harnessInitiatedTurnId == nil else {
            return nil
        }
        let turnId = "background-task:\(taskId)"
        state.harnessInitiatedTurnId = turnId
        states[conversationId] = state
        return .activity(AgentActivityEvent(
            state: .active,
            turnId: turnId,
            metadata: [Self.harnessInitiatedTurnMetadataKey: .bool(true)]
        ))
    }

    /// Ends a harness-initiated turn when Claude answers the notification with a no-op result, which the adapter has
    /// downgraded to interim usage so it cannot end a host-started turn.
    func harnessInitiatedTurnEnd(for event: AgentEvent, conversationId: AgentConversationID) -> AgentEvent? {
        guard case let .usage(usage) = event,
              usage.metadata[AgentBackgroundTaskMetadata.noOpResult] == .bool(true),
              var state = states[conversationId],
              let turnId = state.harnessInitiatedTurnId else {
            return nil
        }
