...
/// Codex harness adapter backed by Codex App Server.
public struct CodexHarnessAdapter: AgentHarnessAdapter {
    /// Codex harness identifier.
    public static let harnessId = CodexHarnessDefinition.harnessId

    /// Configuration used to create a Codex harness adapter.
    ...
    public let definition = CodexHarnessDefinition.definition

    let configuration: Configuration
    private let client: CodexAppServerClient

    /// Creates a Codex harness adapter.
    ...
    private func makeLaunchConfiguration(
        ...
        return AgentLaunchConfiguration(
            ...
            harnessSessionId: bootstrap.threadId,
    ...
    public func performGoalAction(_ action: AgentGoalAction, context: AgentHarnessGoalActionContext) async throws {
    ...
    public func startGoal(_ objective: String, context: AgentHarnessGoalStartContext) async throws {
    ...
    public func archiveSession(_ record: AgentSessionRecord) async throws {
        ...
        for supersededSessionId in record.supersededHarnessSessionIds {
            try await client.archiveThread(supersededSessionId)
        }
        try await client.archiveThread(record.harnessSessionId)
    }
    ...
    public func unarchiveSession(_ record: AgentSessionRecord) async throws {
        try validateSessionActionRecord(record)
        try await client.unarchiveThread(record.harnessSessionId)
    }
    ...
    public func deleteSession(_ record: AgentSessionRecord) async throws {
        ...
        try await client.deleteThread(record.harnessSessionId)
    }
...
extension CodexHarnessAdapter.Configuration {
    func resolvingExecutableIfNeeded(for definition: AgentHarnessDefinition) async -> Self {
