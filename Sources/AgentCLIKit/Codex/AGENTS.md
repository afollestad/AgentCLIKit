...
- `item/reasoning/summaryPartAdded` is the only signal that one summary part ended and the next began; reasoning deltas carry no separator of their own. Decode it as a `"\n\n"` reasoning event for `summaryIndex > 0` so hosts that concatenate deltas keep the part boundary. Codex has no equivalent signal for `textDelta` content parts or for a content/summary kind switch, so those stay unseparated.
