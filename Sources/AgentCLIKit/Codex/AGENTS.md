...
- Fan `archiveSession` and `deleteSession` out over the record's superseded sessions (see `Runtime/AGENTS.md`), archiving those even when deleting — the user retired one conversation, not each thread its forks passed through. `unarchiveSession` stays single-threaded, or restoring a conversation would put every thread it ever forked back into Codex's recents.
