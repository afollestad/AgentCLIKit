...
- Claude honors only the last `--settings` flag, and the hook settings file is appended after the adapter's arguments. Anything a launch passes through `--settings` (the `.shellNetwork` sandbox) must also go into the hook settings file, or starting hooks silently drops it.
