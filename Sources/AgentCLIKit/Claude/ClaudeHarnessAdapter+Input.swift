...
extension ClaudeHarnessAdapter {
    ...
    public func encodeGoalStart(_ objective: String, context: AgentHarnessGoalStartContext) async throws -> AgentHarnessEncodedGoalStart? {
        AgentHarnessEncodedGoalStart(
    ...
    public func availableGoalActions(for goal: AgentGoalSnapshot, context: AgentHarnessGoalActionContext) -> [AgentGoalAction] {
    ...
    public func encodeGoalAction(_ action: AgentGoalAction, context: AgentHarnessGoalActionContext) async throws -> Data? {
        guard action == .delete else {
            throw AgentCLIError.unsupportedCapability(harnessId: Self.harnessId, capability: "goal \(action.rawValue)")
        }
    ...
    public func acceptedSteeringInputEvent(for message: AgentMessageInput, context: AgentHarnessInputContext) -> AgentEvent? {
...
// Mirrors the accessor in `ClaudeHarnessAdapter.swift`; several modules define same-named helpers
