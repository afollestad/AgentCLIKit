...
public struct ClaudeHarnessSetup: AgentHarnessSetup {
    ...
    /// Creates a Claude harness setup service that reports setup readiness from CLI sign-in state.
    ...
    /// Creates a Claude harness setup service.
    ///
    ...
    /// Creates a Claude harness setup service for a Claude config file URL.
    ...
    /// layout, and reporting a gate the probe has not evaluated yet would flicker every harness card
...
private extension ClaudeHarnessSetup {
    ...
    /// Only a verdict the CLI gave us blocks work; `ClaudeAuthReadiness.allowsHarnessWork` owns why
    /// an inconclusive probe still reports ready.
    static func readinessState(for readiness: ClaudeAuthReadiness) -> AgentHarnessReadinessState {
        readiness.allowsHarnessWork ? .ready : .needsSetup
    }
}
