...
/// Runtime context supplied before a harness creates or resumes its native session.
public struct AgentHarnessLaunchContext: Sendable {
    ...
    /// Persisted harness session selected for resume, if any.
    ...
    /// Creates harness launch context.
    public init(
...
extension AgentHarnessLaunchContext {
    func validatedHostToolEndpoint() throws -> AgentHostToolEndpoint? {
        ...
        guard spawnConfig.hostToolServer.hasValidHarnessName else {
        ...
        guard spawnConfig.hostTools.allSatisfy(\.hasValidHarnessName) else {
            throw AgentCLIError.invalidInput(
