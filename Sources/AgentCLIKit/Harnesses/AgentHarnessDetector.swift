...
/// Service that detects whether registered harness executables are available.
public struct AgentHarnessDetector: Sendable {
    ...
    /// Detects availability for a single harness definition.
    public func availability(for definition: AgentHarnessDefinition) async -> AgentHarnessAvailability {
        for executable in definition.executableNames {
            if let path = await resolveExecutable(executable) {
                ...
                return AgentHarnessAvailability(harnessId: definition.id, executablePath: path, versionDescription: version)
            }
        }
        return AgentHarnessAvailability(harnessId: definition.id, executablePath: nil)
    }
    ...
    private func executablePathIfRunnable(_ executable: String) -> String? {
        ...
        // Harness definitions may pin an exact CLI path; those should not depend on PATH lookup.
