...
- Keep persisted harness MCP configuration separate from process-scoped host tools; host tools must never rewrite global harness files.
...
- Preserve trusted conversation, harness, process, and request identity from runtime context; never accept them from model arguments.
