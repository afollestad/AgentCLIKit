...
/// Session lifecycle and transport reuse without starting harness turns.
extension CodexHarnessAdapterTests {
    func testBorrowedRouterCleansUpThroughTheRuntimeTransportAndKeepsItUsable() async throws {
        ...
        let router = AgentHarnessSessionActionRouter(borrowing: AgentHarnessAdapterSet(adapters: [adapter]))
        let config = AgentSpawnConfig(harnessId: .codex, workingDirectory: URL(fileURLWithPath: "/tmp/project"))
    ...
    func testArchivesThreadWithoutRuntimeBootstrap() async throws {
        ...
        try await adapter.archiveSession(sessionRecord(harnessId: .codex, workingDirectory: nil))
    ...
    func testUnarchivesThreadWithoutRuntimeBootstrap() async throws {
        ...
        try await adapter.unarchiveSession(sessionRecord(harnessId: .codex, workingDirectory: nil))
    ...
    func testDeletesThreadWithoutRuntimeBootstrap() async throws {
        ...
        try await adapter.deleteSession(sessionRecord(harnessId: .codex, workingDirectory: nil))
    ...
    func testArchiveThrowsForMismatchedHarnessRecord() async throws {
        let transport = FakeCodexAppServerTransport(threadIds: [])
        let adapter = CodexHarnessAdapter(configuration: configuration(transport: transport))

        do {
            try await adapter.archiveSession(sessionRecord(harnessId: .claude))
    ...
    func testArchiveSurfacesJSONRPCFailure() async throws {
        ...
        do {
            try await adapter.archiveSession(sessionRecord(harnessId: .codex))
    ...
    func testThreadLifecycleActionsSucceedWhenRolloutIsAlreadyGone() async throws {
        let actions: [(method: String, perform: (CodexHarnessAdapter, AgentSessionRecord) async throws -> Void)] = [
        ...
        for action in actions {
            ...
            try await action.perform(adapter, sessionRecord(harnessId: .codex, workingDirectory: nil))
    ...
    func testDeleteSurfacesNonRolloutJSONRPCFailure() async throws {
        ...
        do {
            try await adapter.deleteSession(sessionRecord(harnessId: .codex))
            XCTFail("Expected JSON-RPC failure.")
        } catch let error as CodexAppServerError {
    ...
    func testHarnessResourcesCanShutdownAfterOneShotArchive() async throws {
        let transport = FakeCodexAppServerTransport(threadIds: [])
        let adapter = CodexHarnessAdapter(configuration: configuration(transport: transport))
    ...
    func supersedingSessionRecord(supersededSessionIds: [String]) -> AgentSessionRecord {
        AgentSessionRecord(
            ...
            generation: 0,
            metadata: [
                AgentSessionRecord.supersededHarnessSessionIdsMetadataKey: .array(supersededSessionIds.map(JSONValue.string))
            ]
        )
    }

    func sessionRecord(harnessId: AgentHarnessID, workingDirectory: URL? = URL(fileURLWithPath: "/tmp/project")) -> AgentSessionRecord {
        AgentSessionRecord(
            conversationId: "conversation",
            harnessId: harnessId,
            harnessSessionId: "thread-123",
