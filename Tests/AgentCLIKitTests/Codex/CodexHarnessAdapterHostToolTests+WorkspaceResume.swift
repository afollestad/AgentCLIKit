...
extension CodexHarnessAdapterHostToolTests {
    ...
    func testUnsupportedResumeRootsFailWithoutRetryingOrDroppingGrants() async throws {
        ...
        do {
            _ = try await adapter.makeLaunchConfiguration(
                spawnConfig: AgentSpawnConfig(harnessId: .codex, workingDirectory: directory, additionalWorkspaceRoots: [directory]),
    ...
    func testRelaunchAfterSuspensionReplacesPreviouslyLoadedRootsWithoutHostTools() async throws {
        ...
        XCTAssertEqual(launch.harnessSessionId, "replacement-thread")
...
private extension CodexHarnessAdapterHostToolTests {
    ...
    func workspaceSession(id: String, directory: URL) -> AgentSessionRecord {
        AgentSessionRecord(
            ...
            harnessId: .codex,
            harnessSessionId: AgentSessionID(rawValue: id),
            workingDirectory: directory,
            generation: 1
        )
    }
