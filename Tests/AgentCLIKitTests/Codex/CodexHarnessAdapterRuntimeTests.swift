...
final class CodexHarnessAdapterRuntimeTests: XCTestCase {
    func testLegacyTurnInputRequiresRuntimeContext() async throws {
        let adapter = CodexHarnessAdapter(configuration: configuration(transport: FakeCodexAppServerTransport(threadIds: ["thread-123"])))
    ...
    func testRuntimeCanSendImmediatelyAfterFreshSpawn() async throws {
        ...
        XCTAssertEqual(status?.harnessSessionId, "thread-123")
