...
final class CodexHarnessAdapterHostToolTests: XCTestCase {
    func testStartIncludesWorkspaceRootsAndDottedHostMCPConfig() async throws {
        ...
        let adapter = CodexHarnessAdapter(configuration: Self.configuration(transport: transport))
        ...
        XCTAssertEqual(launch.harnessSessionId, "thread-started")
    ...
    func testReconfigureRequiresRelaunchForRootsHostToolsAndServerMetadata() async throws {
        ...
        var idleResults: [AgentHarnessReconfigureResult] = []
        ...
        var activeResults: [AgentHarnessReconfigureResult] = []
        for updatedConfig in Self.launchOnlyConfigs {
    ...
    func testRuntimeWorkspaceRootsRequireExperimentalAPI() async {
        ...
        let adapter = CodexHarnessAdapter(configuration: Self.configuration(
            transport: transport,
    ...
    private static func reconfigureContext(
        ...
        isTurnActive: Bool
    ) -> AgentHarnessReconfigureContext {
        AgentHarnessReconfigureContext(
            conversationId: "conversation",
            processToken: processToken,
            harnessSessionId: "thread-123",
