...
extension ClaudeHarnessAdapterTests {
    func testDefaultLaunchUsesEnvClaudeFallback() async throws {
        ...
        let adapter = ClaudeHarnessAdapter(configuration: ClaudeHarnessAdapter.Configuration(executableResolver: resolver))
    ...
    func testExactClaudeExecutableBypassesResolver() async throws {
        let resolver = RecordingExecutableResolver(path: "/Users/test/.local/bin/claude")
        let adapter = ClaudeHarnessAdapter(configuration: ClaudeHarnessAdapter.Configuration(
    ...
    func testPrepareLaunchKeepsCompactHooksWhenPermissionModeDisablesApprovalHooks() async throws {
        ...
        let adapter = ClaudeHarnessAdapter(hookSupportDirectory: hookSupportDirectory)
        addTeardownBlock {
    ...
    func testInputEncoderRejectsAttachments() throws {
        ...
        XCTAssertThrowsError(try ClaudeInputEncoder().encode(input)) { error in
            guard case let AgentCLIError.unsupportedInputAttachment(harnessId, attachmentId, type, reason) = error else {
