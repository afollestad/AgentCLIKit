...
final class ClaudeHarnessAdapterTests: XCTestCase {
    ...
    func testClaudeDefinitionExposesHostLaunchMetadata() {
        let definition = ClaudeHarnessAdapter().definition
        ...
        XCTAssertEqual(definition.supportedPermissionModes, ClaudeHarnessDefinition.definition.supportedPermissionModes)
    }

    func testAcceptedSteeringInputEventMarksRuntimeAcceptedInput() {
        let adapter = ClaudeHarnessAdapter(configuration: ClaudeHarnessAdapter.Configuration(enableHooks: false))
    ...
    func testConfigurationInitializerPreservesHookSettings() async throws {
        ...
        let prepared = try await adapter.prepareLaunchConfiguration(
            launch,
            spawnConfig: AgentSpawnConfig(harnessId: .claude, workingDirectory: URL(fileURLWithPath: "/tmp")),
            conversationId: "conversation",
            processToken: UUID()
        )
...
extension ClaudeHarnessAdapterTests {
    static func inputContext(isTurnActive: Bool) -> AgentHarnessInputContext {
    ...
    static func goalActionContext() -> AgentHarnessGoalActionContext {
        goalActionContext(isTurnActive: false)
    }

    static func goalActionContext(isTurnActive: Bool) -> AgentHarnessGoalActionContext {
        AgentHarnessGoalActionContext(
            conversationId: "conversation",
            processToken: UUID(),
            harnessSessionId: "session-id",
            spawnConfig: AgentSpawnConfig(harnessId: .claude, workingDirectory: URL(fileURLWithPath: "/tmp/project")),
    ...
    static func goalStartContext() -> AgentHarnessGoalStartContext {
        AgentHarnessGoalStartContext(
            conversationId: "conversation",
            processToken: UUID(),
            harnessSessionId: "session-id",
            spawnConfig: AgentSpawnConfig(harnessId: .claude, workingDirectory: URL(fileURLWithPath: "/tmp/project")),
