...
/// Covers how `ClaudeHarnessSetup` maps a probe verdict onto `AgentHarnessReadinessState`, which is
/// what every downstream readiness gate reads.
final class ClaudeHarnessSetupAuthTests: XCTestCase {
    ...
    func testSetupWithoutProbeReportsReady() async {
        let setup = ClaudeHarnessSetup(configStore: ClaudeConfigStore(fileURL: Self.scratchConfigURL()))
...
private extension ClaudeHarnessSetupAuthTests {
    static func setup(stdout: String, exitCode: Int32 = 0) -> ClaudeHarnessSetup {
        setup(runner: CountingShellRunner(result: ShellCommandResult(exitCode: exitCode, stdout: stdout, stderr: "")))
    }

    static func setup(runner: any ShellRunning) -> ClaudeHarnessSetup {
        ClaudeHarnessSetup(
