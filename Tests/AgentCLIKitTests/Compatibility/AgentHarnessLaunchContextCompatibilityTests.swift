...
final class HarnessLaunchContextCompatibilityTests: XCTestCase {
    func testLegacyAdapterBridgesContextWithoutRuntimeOwnedCapabilities() async throws {
        let recorder = LegacyLaunchRecorder()
        let adapter = LegacyOnlyHarnessAdapter(recorder: recorder)
        let config = AgentSpawnConfig(harnessId: .claude, workingDirectory: URL(fileURLWithPath: "/tmp/project"))
    ...
    func testLegacyAdapterRejectsHostToolsInsteadOfSilentlyDroppingThem() async {
        let adapter = LegacyOnlyHarnessAdapter(recorder: LegacyLaunchRecorder())
    ...
    func testLegacyAdapterRejectsRegisteredEndpointInsteadOfSilentlyDroppingIt() async throws {
        ...
        do {
            ...
            XCTAssertEqual(
                ...
                .unsupportedCapability(harnessId: .claude, capability: "host tools or additional workspace roots")
            )
        }
    }

    private func assertUnsupportedRuntimeOwnedCapabilities(
        adapter: LegacyOnlyHarnessAdapter,
...
private struct LegacyOnlyHarnessAdapter: AgentHarnessAdapter {
    let definition = AgentHarnessDefinition(id: .claude, displayName: "Legacy", executableNames: ["legacy"])
    let recorder: LegacyLaunchRecorder
