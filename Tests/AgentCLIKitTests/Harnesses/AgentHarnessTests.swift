...
final class AgentHarnessTests: XCTestCase {
    ...
    func testBuiltInHarnessDefinitionsIncludeClaudeAndCodexWithoutRuntimeAdapters() {
        ...
        XCTAssertTrue(definitions[0].capabilities.supportsGoalMode)
        XCTAssertTrue(definitions[0].capabilities.supportsExistingSessionGoalStart)
        XCTAssertEqual(definitions[0].capabilities.supportedGoalActions, [.delete])
        ...
        XCTAssertTrue(definitions[1].capabilities.supportsGoalMode)
        XCTAssertTrue(definitions[1].capabilities.supportsExistingSessionGoalStart)
        XCTAssertEqual(definitions[1].capabilities.supportedGoalActions, [.pause, .resume, .delete])
    ...
    func testBuiltInClaudeHarnessDefinitionExposesPermissionMetadata() {
        XCTAssertEqual(
            ClaudeHarnessDefinition.definition.supportedPermissionModes,
            [
                AgentHarnessOption(
                    value: "default",
                    label: "Default",
    ...
    func testBuiltInCodexHarnessDefinitionExposesPermissionMetadata() {
        XCTAssertEqual(
            CodexHarnessDefinition.definition.supportedPermissionModes,
            [
                AgentHarnessOption(
                    value: "untrusted",
    ...
    func testAdapterSetOverridesBuiltInAdapters() {
        let adapterSet = AgentHarnessAdapterSet(overriding: [
    ...
    func testDefaultAdapterSetAcceptsClaudeConfiguration() async throws {
        let adapterSet = AgentHarnessAdapterSet.default(
            claude: ClaudeHarnessAdapter.Configuration(executablePath: "/custom/claude", enableHooks: false)
        )
        let adapter = try XCTUnwrap(adapterSet.adapters.first)

        let launch = try await adapter.makeLaunchConfiguration(
            spawnConfig: AgentSpawnConfig(harnessId: .claude, workingDirectory: URL(fileURLWithPath: "/tmp/project")),
            resumedSession: nil
        )
    ...
    func testRegistryReturnsRegisteredDefinitions() async {
        let registry = AgentHarnessRegistry.builtIn()
    ...
    func testRegistryUsesLastDuplicateDefinition() async {
        let registry = AgentHarnessRegistry(definitions: [
            AgentHarnessDefinition(id: .claude, displayName: "Old", executableNames: ["old"]),
            AgentHarnessDefinition(id: .claude, displayName: "New", executableNames: ["new"])
        ])
    ...
    func testHarnessDefinitionRoundTripsLaunchMetadataAndDefaultsLegacyFields() throws {
        let definition = AgentHarnessDefinition(
            ...
            supportedPermissionModes: [
                AgentHarnessOption(value: "plan", label: "Plan", description: "Read-only planning.")
            ]
        )

        let data = try JSONEncoder().encode(definition)
        let decoded = try JSONDecoder().decode(AgentHarnessDefinition.self, from: data)
        let legacy = try JSONDecoder().decode(
            AgentHarnessDefinition.self,
        ...
        assertLegacyHarnessDefinitionDefaults(legacy)
    }

    func testHarnessDefinitionsReportLocalImageSupport() {
        XCTAssertTrue(CodexHarnessDefinition.definition.capabilities.supportsLocalImageInput)
        XCTAssertFalse(ClaudeHarnessDefinition.definition.capabilities.supportsLocalImageInput)
    }

    func testHarnessDefinitionsReportSessionDeletionSupport() {
        XCTAssertTrue(CodexHarnessDefinition.definition.capabilities.supportsSessionDeletion)
        XCTAssertFalse(ClaudeHarnessDefinition.definition.capabilities.supportsSessionDeletion)
    }

    func testHarnessCapabilitiesDecodeLegacyModelListingKey() throws {
    ...
    func testModelOptionRoundTripsEffortMetadataAndDefaultsLegacyFields() throws {
        let defaultEffort = AgentHarnessOption(value: "medium", label: "Medium", description: "Balanced reasoning.")
        let option = AgentModelOption(
            ...
            supportedEffortOptions: [
                AgentHarnessOption(value: "low", label: "Low", description: "Faster reasoning."),
                defaultEffort
            ],
        ...
        let legacy = try JSONDecoder().decode(
            ...
            from: Data(#"{"providerId":"codex","id":"default","label":"Harness default","isDefault":true}"#.utf8)
        )
    ...
    func testLaunchConfigurationRoundTripsHarnessSessionIdAndDefaultsLegacyFields() throws {
        ...
        XCTAssertEqual(decoded.harnessSessionId, "harness-session")
        XCTAssertNil(legacy.harnessSessionId)
    ...
    func testDetectorFindsFirstAvailableExecutableAndVersion() async {
        ...
        let detector = AgentHarnessDetector(shellRunner: shell)

        let availability = await detector.availability(
            for: AgentHarnessDefinition(id: .claude, displayName: "Claude", executableNames: ["claude"])
        )
    ...
    func testDetectorReturnsUnavailableWhenNoExecutableIsFound() async {
        ...
        let availability = await detector.availability(
            for: AgentHarnessDefinition(id: .claude, displayName: "Missing", executableNames: ["missing"])
        )
    ...
    func testDetectorAcceptsExecutablePathWithoutWhichLookup() async throws {
        ...
        let availability = await detector.availability(
            for: AgentHarnessDefinition(id: .claude, displayName: "Agent", executableNames: [executableURL.path])
        )
    ...
    func testDetectorResolvesTildeExecutablePath() async throws {
        ...
        let detector = AgentHarnessDetector(shellRunner: shell, homeDirectory: homeDirectory)

        let availability = await detector.availability(
            for: AgentHarnessDefinition(id: .claude, displayName: "Agent", executableNames: ["~/bin/agent"])
        )
    ...
    func testDetectorFallsBackToLoginShellLookup() async throws {
        ...
        let detector = AgentHarnessDetector(
            shellRunner: shell,
    ...
    func testDetectorUsesFallbackExecutableDirectories() async throws {
        ...
        let availability = await detector.availability(
            for: AgentHarnessDefinition(id: .claude, displayName: "Agent", executableNames: ["agent"])
        )
    ...
    func testDefaultExecutableResolverUsesDetectorAvailability() async throws {
        ...
        let resolver = DefaultAgentHarnessExecutableResolver(detector: detector)
    ...
    private func assertLegacyHarnessDefinitionDefaults(_ legacy: AgentHarnessDefinition) {
