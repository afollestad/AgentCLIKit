...
final class AgentModelOptionShortNameTests: XCTestCase {
    ...
    func testHarnessDefaultOptionUsesItsIDAsShortName() {
        let options = AgentDefaultModelOptions.harnessDefault(for: .codex)

        XCTAssertEqual(options.map(\.shortName), ["default"])
    }

    func testOmittedShortNameFallsBackToID() {
        let option = AgentModelOption(harnessId: .codex, id: "gpt-5.5", model: "gpt-5.5", label: "GPT-5.5")
