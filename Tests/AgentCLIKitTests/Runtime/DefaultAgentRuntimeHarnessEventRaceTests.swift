...
final class RuntimeHarnessEventRaceTests: XCTestCase {
    func testDestroyWhileRuntimeEventSubscriptionStartsCancelsStaleStream() async throws {
        let gate = HarnessRuntimeEventStartGate()
        let runtime = DefaultAgentRuntime(adapters: [GatedRuntimeEventHarnessAdapter(gate: gate)])
...
private actor HarnessRuntimeEventStartGate {
...
private struct GatedRuntimeEventHarnessAdapter: AgentHarnessAdapter {
    let gate: HarnessRuntimeEventStartGate
